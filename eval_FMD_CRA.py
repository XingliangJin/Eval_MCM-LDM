...
from src.datasets.get_dataset import get_datasets1, get_datasets1_gt, get_datasets, get_datasets_gen
...
def main():
    ...
    group.add_argument(
        "--pkl_path",
        type=str,
        default="eval_source/content_epoch_399_scale_2-5.pkl",
        required=False,
        help="ply set",
    )
...
# for gt
    ...
    gtparameters = load_args("save_content_recog/opt.yaml")
    gtparameters["device"]=torch.device("cuda:0")
    dataset_name = gtparameters["dataset"]


# load STGCN
    stgcnevaluation = STGCNEvaluation(dataset_name, gtparameters, device=gtparameters["device"], modelpath="save_content_recog/checkpoint_0075.pth.tar")
...
# for gen
    recogparameters = load_args("save_content_recog/opt.yaml")
    ...
    datasetGT = get_datasets_gen(recogparameters)["train"]    
    dataiterator = DataLoader(datasetGT, batch_size=128,
                                    shuffle=False, num_workers=8,
                                    collate_fn=collate)
    ...
    genLoaders = NewDataloader("gen", model, recogparameters,
                                        dataiterator,
                                        recogparameters["device"])
    ...
    print("***********")
    print(stgcn_metrics)
    # save_metrics("eval_results/cra_fid_eval_{}.yaml".format(recogparameters["eval_motion_path"].split('/')[-1].split('.')[0]), stgcn_metrics)

    save_dict_to_file("eval_results/cra_fid_eval_{}.txt".format(recogparameters["eval_motion_path"].split('/')[-1].split('.')[0]), stgcn_metrics)
